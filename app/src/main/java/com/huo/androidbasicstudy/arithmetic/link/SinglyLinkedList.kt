...
//            link.inverseLinkList(link.head?.next?.next)
    ...
    //带结点的链表翻转
    private fun inverseLinkList_head(p: Node): Node {
        //　Head　为新建的一个头结点
    ...
    private fun inverseLinkList(p: Node?): Node? {
        ...
        var curInverseNode = head
        println("z---${curInverseNode?.data}")
        var next: Node?
        while (curInverseNode !== p) {
            next = curInverseNode?.next
            curInverseNode?.next = preNode
            preNode = curInverseNode
            curInverseNode = next
        }
        curInverseNode?.next = preNode
        //　返回左半部分的中点之前的那个节点
        //　从此处开始同步像两边比较
        return curInverseNode
    }
