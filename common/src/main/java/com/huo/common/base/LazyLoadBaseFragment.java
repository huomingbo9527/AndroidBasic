...
import com.ihealth.chronos.patient.base.util.LogUtil;
...
public  class LazyLoadBaseFragment extends Fragment {
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        // 这种情况下第一次可见不是在这里通知 因为 isViewCreated = false 成立,等从别的界面回到这里后会使用 onResumeLazy 通知可见
    ...
    public void onHiddenChanged(boolean hidden) {
        ...
        LogUtil.e(getClass().getSimpleName() + "  onHiddenChanged dispatchChildVisibleState  hidden " + hidden);
    ...
    private void dispatchUserVisibleHint(boolean visible) {
        if(!isViewCreated)//没创建直接返回  解决派发子dispatchUserVisibleHint，子类没初始化的问题
            return;
...
//        // 当父 dispatchChildVisibleState 的时候第二次回调本方法 visible = false 所以此处 visible 将直接返回
    ...
    protected void lazyLoad() {
        LogUtil.e(getClass().getSimpleName() + "  对用户第一次可见");

    }

    protected void onResumeLazy() {
        LogUtil.e(getClass().getSimpleName() + "  对用户可见");
    }

    protected void onPauseLazy() {
        LogUtil.e(getClass().getSimpleName() + "  对用户不可见");
    }
